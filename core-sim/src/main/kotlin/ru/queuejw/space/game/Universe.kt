...
fun starColor(cls: StarClass) =
    when (cls) {
        StarClass.O -> KColor.fromArgb(0xFF6666FF.toInt())
        StarClass.B -> KColor.fromArgb(0xFFCCCCFF.toInt())
        StarClass.A -> KColor.fromArgb(0xFFEEEEFF.toInt())
        StarClass.F -> KColor.fromArgb(0xFFFFFFFF.toInt())
        StarClass.G -> KColor.fromArgb(0xFFFFFF66.toInt())
        StarClass.K -> KColor.fromArgb(0xFFFFCC33.toInt())
        StarClass.M -> KColor.fromArgb(0xFFFF8800.toInt())
    }
...
class Star(val cls: StarClass, radius: Float) :
    ...
    var anim = 0f
    override fun update(sim: Simulator, dt: Float) { anim += dt }
}
...
open class Universe(val namer: INamer, randomSeed: Long) : Simulator(randomSeed) {
    ...
    fun initTest() {
        ...
        star = Star(cls = StarClass.A, radius = STAR_RADIUS_RANGE.endInclusive).apply { name = "TEST SYSTEM" }

        repeat(NUM_PLANETS_RANGE.last) {
            val thisPlanetFrac = it.toFloat() / (NUM_PLANETS_RANGE.last - 1)
            val radius = lerp(PLANET_RADIUS_RANGE.start, PLANET_RADIUS_RANGE.endInclusive, thisPlanetFrac)
            val orbitRadius = lerp(PLANET_ORBIT_RANGE.start, PLANET_ORBIT_RANGE.endInclusive, thisPlanetFrac)
            ...
            val p = Planet(
                ...
                speed = speed,
                color = KColor.fromArgb(0xFFA7A7CA.toInt())
            )
            val num = it + 1
    ...
    fun initRandom() {
        ...
        star = Star(cls = rng.choose(StarClass.values()), radius = rng.nextFloatInRange(STAR_RADIUS_RANGE))
        ...
        repeat(rng.nextInt(NUM_PLANETS_RANGE.first, NUM_PLANETS_RANGE.last + 1)) {
            ...
            val orbitRadius = lerp(PLANET_ORBIT_RANGE.start, PLANET_ORBIT_RANGE.endInclusive, rng.nextFloat().pow(1f))
        ...
        ship = Spacecraft()
        ship.pos = star.pos + Vec2.makeWithAngleMag(rng.nextFloat() * PI2f, rng.nextFloatInRange(PLANET_ORBIT_RANGE.start, PLANET_ORBIT_RANGE.endInclusive))
    ...
    override fun updateAll(dt: Float, entities: ArraySet<Entity>) {
        ...
        (planets + star).forEach { planet ->
            ...
            if (d < planet.radius) {
                ...
                ship.velocity = ship.velocity + Vec2.makeWithAngleMag(vector.angle(), GRAVITATION * (ship.mass * planet.mass) / d.pow(2)) * dt
            }
        }
    ...
    fun closestPlanet(): Planet {
        val bodiesByDist = (planets + star).map { planet -> (planet.pos - ship.pos) to planet }.sortedBy { it.first.mag() }
            ...
            if (planet.collides) {
                ...
                if (d < 0) {
                    ...
                    if (aDiff < PIf / 4) {
                        ...
                        ship.pos = planet.pos + Vec2.makeWithAngleMag(a, planet.radius + ship.radius - d)
                        (1..10).forEach {
                            Spark(ttl = rng.nextFloatInRange(0.5f, 2f), style = Spark.Style.DOT, color = KColor.White, size = 1f).apply {
                                pos = impact + Vec2.makeWithAngleMag(rng.nextFloatInRange(0f, 2 * PIf), rng.nextFloatInRange(0.1f, 0.5f))
                                opos = pos
                                velocity = ship.velocity * 0.8f + Vec2.makeWithAngleMag(rng.nextFloatInRange(0f, 2 * PIf), rng.nextFloatInRange(0.1f, 0.5f))
                                add(this)
                            }
                        }
                    }
...
class Spark(
    ...
    val color: KColor = KColor.Gray,
    val size: Float = 2f,
    val fuse: Fuse = Fuse(ttl)
) : Removable by fuse, Body(name = "Spark") {
    enum class Style { LINE, LINE_ABSOLUTE, DOT, DOT_ABSOLUTE, RING }
    init { this.collides = collides; this.mass = mass }
    override fun update(sim: Simulator, dt: Float) { super.update(sim, dt); fuse.update(dt) }
}
        ...
        if (sim.rng.nextFloat() < mag) {
            sim.add(
                Spark(
                    ...
                    size = 1f,
                    color = KColor(1f, 1f, 1f, 0.25f)
                ).also { spark ->
                    spark.pos = pos
                    spark.opos = pos
                    spark.velocity = velocity + Vec2.makeWithAngleMag(
